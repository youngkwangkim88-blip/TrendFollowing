...
- 자본 업데이트:
  ...
  - **Unit 사이즈 산정에 사용하는 `M`도 연 단위로 리베이스**한다(= 연복리 반영).
    - 백테스트: 연말 마지막 거래일 EOD NAV를 다음 해의 `M`으로 사용.
...
- 포지션 관리 기법(필수):
  ...
  - even-stop(브레이크이븐 스탑): +10% MFE 후 진입가 회귀 시 청산
  - emergency stop (TS 타입과 무관, 항상 적용 / 3종):
    - ES1: 당일 시가(open) 기준 ±5% 역방향이면 장중 청산
    - ES2: 전일 종가(close) 기준 ±5% 역방향을 장중 터치하면 청산(GAP/TOUCH)
    - ES3: 종가-종가(close-to-close) 기준 ±5% 역방향이면 다음 거래일 시가 청산
  - 피라미딩(Pyramiding)
